const { SlashCommandBuilder, ChatInputCommandInteraction, Client, EmbedBuilder, PermissionFlagsBits, time } = require("discord.js");
const { Types } = require("mongoose");  
const warnSchema = require("../../database/warnSchema");
const modSchema = require("../../database/modSchema");
        ...
        case "add":
          {
            ...
            if (modData) {
              client.channels.cache.get(modData.channelId).send({
                embeds: [
                  new EmbedBuilder().setTitle("A user has been warned").setColor(config.embed.color).addFields(
                    {
                      name: "User warned",
                      value: `<@${user.id}>`,
                      inline: true,
